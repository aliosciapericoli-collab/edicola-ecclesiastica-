...
Repository autonomo, deploy in `/home/work/edicola-ecclesiastica`, processo
PM2 `edicola-ecclesiastica` sulla porta **3202**, database propri in `data/`.
Stessa architettura del progetto d'origine: server Node monolitico + SPA React
+ SQLite + pipeline AI "Scalata".
...
Repository dedicato, clonato in una cartella propria:

```bash
cd /home/work
git clone https://github.com/aliosciapericoli-collab/edicola-ecclesiastica.git
cd edicola-ecclesiastica
...
# 2. Chiavi API (stesse del progetto madre o dedicate)
cp /home/work/edicola-giuridica/.env .env

# 3. Schema DB notizie copiato dal madre (percorso già in ecosystem.config.js)
...
I percorsi verso i DB del progetto madre (filtro giurisprudenza, schema
notizie) sono in `ecosystem.config.js` (`ECCL_CASSAZIONE_SRC`,
`ECCL_GIURIDICA_SRC`): se il madre non c'è, il filtro giurisprudenza
semplicemente salta e il resto funziona lo stesso.
...
poi `sudo systemctl reload caddy`.
